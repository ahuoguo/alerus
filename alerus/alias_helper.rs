...
pub proof fn lemma_bin_contrib_sum_sel(t: Alias, e: spec_fn(nat) -> real, i: nat, n: nat)
    ensures
        bin_contrib_sum(t, e, i, n)
            == (if i < n { (t.prob)(i) as real * e(i) } else { 0real })
             + (if (t.alias)(i) < n { ((t.m - (t.prob)(i)) as nat) as real * e((t.alias)(i)) } else { 0real }),
    decreases n,
{
    if n > 0 {
        let ghost km = (n - 1) as nat;
        lemma_bin_contrib_sum_sel(t, e, i, km);
        let ghost vp = (t.prob)(i) as real * e(i);
        let ghost va = ((t.m - (t.prob)(i)) as nat) as real * e((t.alias)(i));
        ...
        assert(e(km) * (bin_contrib(t, i, km) as real)
            ...
            requires
                ...
                i == km ==> e(km) == e(i),
                (t.alias)(i) == km ==> e(km) == e((t.alias)(i)),
                vp == (t.prob)(i) as real * e(i),
                va == ((t.m - (t.prob)(i)) as nat) as real * e((t.alias)(i));
    }
}

/// Adding bin j−1 to the label-grouped view adds its bin_contrib_sum.
pub proof fn lemma_label_credit_sum_step(t: Alias, e: spec_fn(nat) -> real, j: nat, n: nat)
    requires j >= 1,
    ...
    decreases n,
{
    if n > 0 {
        ...
        assert(e((n - 1) as nat) * (label_units(t, j, km) as real)
            == e((n - 1) as nat) * (label_units(t, (j - 1) as nat, km) as real)
             + e((n - 1) as nat) * (bin_contrib(t, (j - 1) as nat, km) as real))
            by(nonlinear_arith)
...
pub proof fn lemma_label_credit_sum_zero(t: Alias, e: spec_fn(nat) -> real, n: nat)
    ...
    decreases n,
{
    if n > 0 {
        ...
        assert(e((n - 1) as nat) * (label_units(t, 0, (n - 1) as nat) as real) == 0real) by(nonlinear_arith)
            requires label_units(t, 0, (n - 1) as nat) == 0;
    }
}

/// Finite Fubini:  the bin-grouped sum equals the label-grouped sum.
pub proof fn lemma_fubini(t: Alias, e: spec_fn(nat) -> real, j: nat)
...
pub proof fn lemma_label_credit_sum_validity(t: Alias, e: spec_fn(nat) -> real, n: nat)
...
pub proof fn lemma_bin_sum_eq(t: Alias, e: spec_fn(nat) -> real)
...
pub proof fn lemma_inner_eps_nonneg(t: Alias, e: spec_fn(nat) -> real, i: nat)
    ...
    ensures inner_eps(t, e, i) >= 0real,
{
    assert(bin_credit(t, e, i) >= 0real) by(nonlinear_arith)
        requires e(i) >= 0real, e((t.alias)(i)) >= 0real;
...
pub proof fn lemma_oalloc_sum(t: Alias, e: spec_fn(nat) -> real, j: nat)
    requires t.m >= 1,
    ...
    decreases j,
{
    if j > 0 {
        ...
        assert((oalloc(t, e))((j - 1) as nat) == inner_eps(t, e, (j - 1) as nat));   // closure β
        ...
        assert(sum_credit(oalloc(t, e), j) == bin_sum(t, e, j) / (t.m as real)) by(nonlinear_arith)
            requires
                ...
                (oalloc(t, e))((j - 1) as nat) == bin_credit(t, e, (j - 1) as nat) / (t.m as real),
                sum_credit(oalloc(t, e), j)
                    == sum_credit(oalloc(t, e), (j - 1) as nat) + (oalloc(t, e))((j - 1) as nat),
...
pub proof fn lemma_average_outer(t: Alias, e: spec_fn(nat) -> real)
    requires valid_alias(t),
...
pub proof fn lemma_ialloc_stepsum(t: Alias, e: spec_fn(nat) -> real, i: nat, m: nat)
    ensures
        sum_credit(ialloc(t, e, i), m)
            == (if m <= (t.prob)(i) { m as real * e(i) }
                else { (t.prob)(i) as real * e(i)
                       + (m - (t.prob)(i)) as real * e((t.alias)(i)) }),
    decreases m,
{
    if m > 0 {
        ...
        let ghost ei = e(i);
        let ghost ea = e((t.alias)(i));
        lemma_ialloc_stepsum(t, e, i, km);                   // IH at km
        if km < p {
            assert((ialloc(t, e, i))(km) == ei) by { assert(km < p); }
            ...
            assert((ialloc(t, e, i))(km) == ea) by { assert(km >= p); }
        ...
        assert(m as real * e(i) == 0real) by(nonlinear_arith) requires m == 0;
    }
}

/// The inner draw's average at bin i equals inner_eps(i).
pub proof fn lemma_inner_average(t: Alias, e: spec_fn(nat) -> real, i: nat)
